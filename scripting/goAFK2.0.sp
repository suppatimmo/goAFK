...
bool g_bHasRoundJustEnded;
bool g_bExcludeBotsFromChecking;
bool g_bIsBombPlanted;
...
public void OnPluginStart() {
    ...
    g_cvPluginMode = CreateConVar("goafk_mode", "1", "1 - KICK / 2 - MOVE TO SPECT")
    ...
    g_cvMoveTime = CreateConVar("goafk_movetime", "150.0", "Time to move player");
    g_cvKickTime = CreateConVar("goafk_kicktime", "300.0", "Time to kick player");
    g_cvWarnTime = CreateConVar("goafk_warntime", "120.0", "Time to warn player");    
    ...
    g_cvAdminImmunityFlag = CreateConVar("goafk_adminflag", "c", "Admin flag for immunity, blank = any flag");
...
public void Event_RoundStart(Handle event, const char[] name, bool dontBroadcast) {
    g_bHasRoundJustEnded = false;
    g_bIsBombPlanted = false;
}

public void Event_RoundEnd(Handle event, const char[] name, bool dontBroadcast) {
    g_bHasRoundJustEnded = true;
}
...
void CheckAlivePlayers() {
    int CT_Players = GetPlayersAlive(CS_TEAM_CT);
    int T_Players = GetPlayersAlive(CS_TEAM_T);
    int players = CT_Players + T_Players;
    if(!g_bHasRoundJustEnded) {
        if(players <= 0)
            CS_TerminateRound(1.0, CSRoundEnd_Draw);
        else if(CT_Players == 0)
            CS_TerminateRound(1.0, CSRoundEnd_TerroristWin);
        else if(T_Players == 0 && !g_bIsBombPlanted)
